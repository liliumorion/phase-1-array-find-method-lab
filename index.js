//superbowlWin(record): returns a year the Denver Broncos won the superbowl, & returns undefined if the record has no win objects.

const superbowlWin = function(record){
  const denverWin = record.find(function(item){
    return item.result === "W";
  })
    
  if (denverWin){return denverWin.year };
}
const record = [
